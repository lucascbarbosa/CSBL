...
top_features_file = Path(input_file_X.parents[1],'Top_Features/'+pathX[:-4]+'_'+coly+'_TOP%d.'%(top_max)+file_format)
features_score_file = Path(input_file_X.parents[1],'Features_Score/'+pathX[:-4]+'_'+coly+'_TOP%d_SCORES.'%(top_max)+file_format)


input2arff(input_file_X,input_file_y,filtered_input_file,coly,file_format)
command_top = 'python get_best_top.py %s %s %d %d %f'%(pathX,coly,trees,top_max,test_size)
sub = Popen(command_top, shell=True, stdout=PIPE, stderr=STDOUT)
top = int(sub.stdout.read())
print(top)
X = top_features(input_file_X,coly,file_format,top,input_file_y,filtered_input_file,top_features_file)
