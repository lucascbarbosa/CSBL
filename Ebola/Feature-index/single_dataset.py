...
top_features_file = Path(input_file_X.parents[1],'Top_Features/'+pathX[:-4]+'_TOP%d.'%(top)+file_format)
features_score_file = Path(input_file_X.parents[1],'Features_Score/'+pathX[:-4]+'_TOP%d_SCORES.'%(top)+file_format)

input2arff(input_file_X,input_file_y,filtered_input_file,coly,file_format)
top_features(input_file_X,input_file_y,filtered_input_file,top_features_file,coly,file_format,top)
